...
import 'package:module_settings/mine/mine_api.dart';
...
class MineHttpTestPage extends StatefulWidget {
  const MineHttpTestPage({super.key});

  @override
  State<MineHttpTestPage> createState() => _MineHttpTestPageState();
}

class _MineHttpTestPageState extends State<MineHttpTestPage> {
  final _args = MineHttpTestArgs.empty();

  _LoadState _loadState = _LoadState.loading;
  HarmonyIndexModel? _indexModel;
  ...
  void didChangeDependencies() {
    ...
    _args.updateFromRoute(ModalRoute.of(context)?.settings.arguments);
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final data = await MineApi.fetchHarmonyIndex();
      if (!mounted) return;
      setState(() {
        _indexModel = data;
  ...
  List<HarmonySectionModel> get _visibleSections {
    final model = _indexModel;
    if (model == null) return const [];

    final section = _args.sectionForLevel(model);
              ...
              child: ListView(
                ...
                children: [
                  _RequestInfoCard(
                    ...
                    level: _args.level,
                    loadedAt: _loadedAt,
                    articleCount: _indexModel?.totalArticleCount,
                  ),
                  ...
                  if (_loadState == _LoadState.loading)
                    ...
                    _ErrorPanel(
                      message: _errorMessage ?? '请求失败',
                      onRetry: _loadData,
                    )
                  else ...[
                    if (_args.level >= 4) _OverviewBanner(model: _indexModel!),
...
class MineHttpTestArgs {
  ...
  HarmonySectionModel? sectionForLevel(HarmonyIndexModel model) {
    switch (level) {
      case 1:
        return model.links;
      case 2:
        return model.openSources;
      case 3:
        return model.tools;
      default:
        return null;
...
class _RequestInfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final status = switch (loadState) {
      _LoadState.loading => _StatusInfo('请求中', const Color(0xFFFFB020)),
      _LoadState.success => _StatusInfo('请求成功', const Color(0xFF53D65B)),
      _LoadState.error => _StatusInfo('请求失败', const Color(0xFFFF6B6B)),
    };
            ...
            children: [
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      loadState == _LoadState.loading
                          ? Icons.sync_rounded
                          : loadState == _LoadState.success
