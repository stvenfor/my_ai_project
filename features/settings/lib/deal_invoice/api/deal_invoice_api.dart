...
class DealInvoiceApi {
  ...
  Future<DealInvoiceSummary> fetchSummary() {
    return _guarded(() async {
      final result =
          await HttpManager.instance.get<ResultModel<DealInvoiceSummary>>(
        '$_base/summary',
  ...
  Future<DealInvoiceItem> fetchDetail(String invoiceId) {
    return _guarded(() async {
      final result =
          await HttpManager.instance.get<ResultModel<DealInvoiceItem>>(
        '$_base/$invoiceId',
        converter: (json) => ResultModel.fromJson(
          json as Map<String, dynamic>,
  ...
  Future<DealInvoiceItem> create({
    ...
    return _guarded(() async {
      final result =
          await HttpManager.instance.post<ResultModel<DealInvoiceItem>>(
        _base,
        data: {
