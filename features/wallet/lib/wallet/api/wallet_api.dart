...
class WalletApi {
  ...
  Future<List<WalletLedgerEntry>> fetchLedger({int limit = 20, int offset = 0}) async {
    AuthHttpConfig.ensureInitialized();
    final result = await HttpManager.instance.get<ResultModel<List<WalletLedgerEntry>>>(
      '$base/ledger',
      ...
      converter: (json) {
        ...
        if (map.containsKey('code')) {
          return ResultModel(code: (map['code'] as num?)?.toInt() ?? 0, message: map['message']?.toString() ?? '', data: list);
        }
  ...
  Future<WalletRechargeResult> recharge({
    ...
    AuthHttpConfig.ensureInitialized();
    final result = await HttpManager.instance.post<ResultModel<WalletRechargeResult>>(
      '$base/recharge',
      data: {
        'amount': amount,
        'channel': channel,
