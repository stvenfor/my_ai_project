...
/// 顶栏：展示名 / 职务 / 店名 / 头像 + 四格统计（对标成交）。
class NewCarFollowProfileHeader extends StatelessWidget {
  const NewCarFollowProfileHeader({super.key, this.summary});

  final NewCarFollowSummary? summary;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              _StatCell(value: '${stats.active}', label: '跟进中'),
              _StatCell(value: '${stats.overdue}', label: '逾期'),
              _StatCell(value: '${stats.highIntent}', label: '高意向'),
              _StatCell(value: '${stats.lost}', label: '战败'),
            ],
          ),
        ],
      ),
